...
void PerfEvaluator::eval(){
	...
	for (unsigned int nbResets = 0; nbResets < 5; ++nbResets){
		PTGFactory factory;
		PTG* ptg = factory.build(20,100,nbResets,5,5,3);
		PTGSolver solver;
		gettimeofday(&start, NULL);
		...
		cerr << nbResets << ":" << 1000 * (end.tv_sec - start.tv_sec) + (end.tv_usec - start.tv_usec)/1000 << "ms" << endl;
		usleep(300000);
